...
class SeedDMS_Core_Role { /* {{{ */
	...
	/**
	 * @var string role of user. Can be one of SeedDMS_Core_Role::role_user,
	 *      SeedDMS_Core_Role::role_admin, SeedDMS_Core_Role::role_guest
	 *
	 * @access protected
	 */
	protected $_role;
	...
	public function __construct($id, $name, $role, $noaccess = array()) { /* {{{ */
		$this->_id = $id;
		$this->_name = $name;
		$this->_role = $role;
	...
	public function isAdmin() { return ($this->_role == SeedDMS_Core_Role::role_admin); }

	public function isGuest() { return ($this->_role == SeedDMS_Core_Role::role_guest); }

	public function isUser() { return ($this->_role == SeedDMS_Core_Role::role_user); }
	...
	public function getNoAccess() { return $this->_noaccess; }
...
class SeedDMS_Core_User { /* {{{ */
	 ...
	 * @param string $secret
	 */
	public function __construct($id, $login, $pwd, $fullName, $email, $language, $theme, $comment, $role, $isHidden = 0, $isDisabled = 0, $pwdExpiration = '', $loginFailures = 0, $quota = 0, $homeFolder = null, $secret = '') {
		$this->_id = $id;
		$this->_login = $login;
		$this->_pwd = $pwd;
	...
	public static function getInstance($id, $dms, $by = '', $email = '') { /* {{{ */
		...
		$user = new self((int) $resArr["id"], $resArr["login"], $resArr["pwd"], $resArr["fullName"], $resArr["email"], $resArr["language"], $resArr["theme"], $resArr["comment"], $role, $resArr["hidden"], $resArr["disabled"], $resArr["pwdExpiration"], $resArr["loginfailures"], (int) $resArr["quota"], $resArr["homefolder"], $resArr["secret"]);
		$user->setDMS($dms);
		return $user;
	} /* }}} */
	...
	public static function getAllInstances($orderby, $dms) { /* {{{ */
		...
		for ($i = 0; $i < count($resArr); $i++) {
			...
			$user = new self($resArr[$i]["id"], $resArr[$i]["login"], $resArr[$i]["pwd"], $resArr[$i]["fullName"], $resArr[$i]["email"], (isset($resArr[$i]["language"])?$resArr[$i]["language"]:null), (isset($resArr[$i]["theme"])?$resArr[$i]["theme"]:null), $resArr[$i]["comment"], $role, $resArr[$i]["hidden"], $resArr[$i]["disabled"], $resArr[$i]["pwdExpiration"], $resArr[$i]["loginfailures"], (int) $resArr[$i]["quota"], $resArr[$i]["homefolder"]);
			$user->setDMS($dms);
			$users[$i] = $user;
		}
	 ...
	 *
	 * @param $newPwd new encrypted password
	 * @return bool
	 */
	 ...
	 * @return string
	 */
	public function getPwdExpiration() { return $this->_pwdExpiration; }

	/**
	 * @param $newPwdExpiration
	 * @return bool
	 */
	public function setPwdExpiration($newPwdExpiration) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return bool
	 */
	public function isAdmin() { return (is_object($this->_role) ? $this->_role->isAdmin() : $this->_role == SeedDMS_Core_Role::role_admin); }
	 ...
	 * @return bool
	 */
	public function isGuest() { return (is_object($this->_role) ? $this->_role->isGuest() : $this->_role == SeedDMS_Core_Role::role_guest); }
	 ...
	 * @return bool
	 */
	public function isUser() { return (is_object($this->_role) ? $this->_role->isUser() : $this->_role == SeedDMS_Core_Role::role_user); }
	 ...
	 * @return bool
	 */
	public function isHidden() { return $this->_isHidden; }
	 ...
	 * @return bool|int
	 */
	public function isDisabled() { return $this->_isDisabled; }
	 ...
	 * @return bool
	 */
	public function clearLoginFailures() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return int
	 */
	public function getQuota() { return $this->_quota; }
	 ...
	 */
	public function getHomeFolder() { return $this->_homeFolder; }
	 ...
	 * @return boolean true on success or false in case of an error
	 */
	public function remove($user, $assignToUser = null) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return bool
	 */
	public function setImage($tmpfile, $mimeType) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function getDocumentsLocked() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function getDocumentsCheckOut() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function getReceiptStatus($documentID = null, $version = null) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function getRevisionStatus($documentID = null, $version = null) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return array|bool list of all workflows
	 */
	public function getWorkflowsInvolved() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function getMandatoryReviewers() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function isMandatoryReviewerOf() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function isMandatoryApproverOf() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function getMandatoryWorkflow() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return SeedDMS_Core_Workflow[]|bool workflow
	 */
	public function getMandatoryWorkflows() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function setMandatoryReviewer($id, $isgroup = false) { /* {{{ */
		$db = $this->_dms->getDB();
		$id = (int) $id;
	 ...
	 * @return boolean true on success, otherwise false
	 */
	public function setMandatoryApprover($id, $isgroup = false) { /* {{{ */
		$db = $this->_dms->getDB();
		$id = (int) $id;
	 ...
	 * @return boolean true on success, otherwise false
	 */
	public function setMandatoryWorkflow($workflow) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true on success, otherwise false
	 */
	public function setMandatoryWorkflows($workflows) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true on success, otherwise false
	 */
	public function delMandatoryReviewers() { /* {{{ */
		$db = $this->_dms->getDB();
		$queryStr = "DELETE FROM `tblMandatoryReviewers` WHERE `userID` = " . $this->_id;
	 ...
	 * @return boolean true on success, otherwise false
	 */
	public function delMandatoryApprovers() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true on success, otherwise false
	 */
	public function delMandatoryWorkflow() { /* {{{ */
		$db = $this->_dms->getDB();
		$queryStr = "DELETE FROM `tblWorkflowMandatoryWorkflow` WHERE `userid` = " . $this->_id;
		if (!$db->getResult($queryStr)) return false;
		return true;
	 ...
	 * @return array list of users
	 */
	public function getReverseSubstitutes() { /* {{{ */
		$db = $this->_dms->getDB();
		...
		if (!isset($this->_rev_substitutes)) {
			 ...
			 */
			if (!$this->isAdmin())
				$queryStr .= " AND `tblRoles`.`role` = ".SeedDMS_Core_Role::role_user;
			$resArr = $db->getResultArray($queryStr);
			if (is_bool($resArr) && $resArr == false)
				return false;
	 ...
	 * @return boolean true if successful otherwise false
	 */
	public function isSubstitute($substitute) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true if successful otherwise false
	 */
	public function maySwitchToUser($touser) { /* {{{ */
		$db = $this->_dms->getDB();
