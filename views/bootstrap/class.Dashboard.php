...
class SeedDMS_View_Dashboard extends SeedDMS_Theme_Style
{
    ...
    private function _renderChartAndTable($chartType, $allChartData, $dms) {
        ...
        if (!$this->showChartCondition($chartType, $dms)) return;
        ...
        if (!empty($currentChartDataForTable)) {
            ...
            foreach($currentChartDataForTable as $item) {
                echo "<tr>";
                $itemKey = $item['key'] ?? 'N/A'; $itemTotal = $item['total'] ?? 0;
                ...
                } else { echo "<td class='text-right'>".(int)$itemTotal."</td>"; }
                if(in_array($chartType, ['docspermonth'])) {
                     echo "<td class='text-right'>".sprintf('%+d', (int)$itemTotal - (int)$oldtotal)."</td>";
                }
                $oldtotal = $itemTotal; $grandTotal += (float)$itemTotal;
                echo "</tr>";
            }
            ...
            if (in_array($chartType, ['sizeperuser', 'sizepermonth'])) { echo SeedDMS_Core_File::format_filesize($grandTotal); }
            elseif ($chartType == 'docsaccumulated'){ echo (int)$oldtotal; }
            else { echo (int)$grandTotal; }
            echo "</td>";
            if(in_array($chartType, ['docspermonth'])) echo "<td></td>";
	...
	protected function printList($documents, $previewer) {
        ...
        if(is_string($headerTxt)) {
            ...
            echo "<thead>\n<tr>\n";
            foreach($headerTxt as $headcol) echo "<th>".htmlspecialchars($headcol)."</th>\n";
            echo "</tr>\n</thead>\n";
        } else {
        ...
        if (empty($documents)) {
            ...
            if(is_array($headerTxt)) $colspan = count($headerTxt);
            else if(is_string($headerTxt) && preg_match_all('/<th[^>]*>/i', $headerTxt, $matches)) $colspan = count($matches[0]);
            ...
            foreach($documents as $document) {
                ...
                if(is_string($itemTxt)) {
                    ...
                    $extracontent = [];
                    if(method_exists($this, 'getListRowPath')) { // Check if method exists
        ...
        if(is_string($footerTxt)) echo $footerTxt;
        else echo "</tbody>\n</table>\n";
	}
	...
	public function newdocuments() {
		...
		if($conversionmgr) $previewer->setConversionMgr($conversionmgr);
		else $previewer->setConverters($previewconverters);
    ...
    function js() {
        ...
        if (is_array($mlMonthNames) && count($mlMonthNames) == 12) $monthNamesJs = array_values($mlMonthNames);
        ...
        jQuery(document).ready(function($) {
            ...
            <?php if ($enableDropUploadOnDashboard && $dashboardUploadFolder): ?>
                ...
                console.log("Dashboard Drop Upload: enableDropUploadOnDashboard=true, dashboardUploadFolder is set. uploadFolderId:", uploadFolderId);
                ...
                if (typeof SeedDMSUpload !== 'undefined' && uploadFolderId !== null) {
                    ...
                    console.error("Dashboard Drop Upload: SeedDMSUpload library not found or uploadFolderId is invalid/null.", "SeedDMSUpload defined?", (typeof SeedDMSUpload !== 'undefined'), "uploadFolderId:", uploadFolderId);
                    if ($('#dashboard-drop-zone').length) { // Check if div exists to put message
                         $('#dashboard-drop-zone').html("<p style='color:red; padding-top: 50px; text-align:center;'><em><?php echo htmlspecialchars(addslashes(getMLText('error_drop_upload_misconfigured', [], 'File drop upload is misconfigured or target folder is invalid.')), ENT_QUOTES, 'UTF-8'); ?></em></p>");
                    }
                }
            <?php else: ?>
                console.log("Dashboard Drop Upload: Conditions not met. enableDropUploadOnDashboard:", <?php echo json_encode($enableDropUploadOnDashboard); ?>, "dashboardUploadFolder set:", <?php echo json_encode(!!$dashboardUploadFolder); ?>);
                 if ($('#dashboard-drop-zone').length && <?php echo json_encode($enableDropUploadOnDashboard); ?> && !<?php echo json_encode(!!$dashboardUploadFolder); ?>) {
                     $('#dashboard-drop-zone').html("<p style='color:orange; padding-top: 50px; text-align:center;'><em><?php echo htmlspecialchars(addslashes(getMLText('info_drop_upload_no_folder', [], 'Quick upload enabled, but no target folder specified.')), ENT_QUOTES, 'UTF-8'); ?></em></p>");
                 }
            <?php endif; ?>
            ...
            if (typeof $.plot === 'undefined') {
                ...
                // console.log("VIEW JS: Final allChartsData passed to Flot:", JSON.parse(JSON.stringify(allChartsDataFromPHP))); // Optional: less verbose
                        ...
                        if (flotDataSeries && (Array.isArray(flotDataSeries) && flotDataSeries.length > 0) ) {
                             if(chartInfo.type === 'docsperuser' || chartInfo.type === 'docsperstatus'){
                                ...
                                if(!hasActualData && flotDataSeries.length > 0){ 
                                    // console.warn("FlotDataSeries for PIE chart " + chartInfo.type + " has only zero or no data points.", flotDataSeries); // Optional: less verbose
                                     $(chartDivSelector).html("<p style='text-align:center; padding-top:50px;'>" + noDataMessage + "</p>");
                                     return;
                                }
                            }
                            // console.log("Plotting chart: " + chartInfo.type + " with data: ", JSON.parse(JSON.stringify(flotDataSeries)), "and options: ", plotOptions); // Optional: less verbose
                            $.plot(chartDivSelector, flotDataSeries, plotOptions);
                        } else {
                             // console.warn("FlotDataSeries is empty or not plottable for " + chartInfo.type, phpStyleDataForChart, flotDataSeries); // Optional: less verbose
                             $(chartDivSelector).html("<p style='text-align:center; padding-top:50px;'>" + noDataMessage + "</p>");
                        }
                    });
                }
...
public function show() {
        ...
        $this->rowStart();
            $this->columnStart(12); 
                ...
                $diskSpaceUsed = 'N/A';
                 if ($settings) {
                    if (isset($settings->storageUsed)) $diskSpaceUsed = SeedDMS_Core_File::formatFileSize($settings->storageUsed);
                    elseif (isset($settings->_storageUsed)) $diskSpaceUsed = SeedDMS_Core_File::formatFileSize($settings->_storageUsed);
                }
                $statsData = [
