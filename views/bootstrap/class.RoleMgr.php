...
class SeedDMS_View_RoleMgr extends SeedDMS_Theme_Style
{
	...
	{ /* {{{ */
		...
		if ($selrole) {
			...
			if ($users) {
				"<table class=\"table table-condensed table-sm\"><thead><tr><th>" . getMLText('name') . "</th><th></th></tr></thead><tbody>\n";
	...
	{ /* {{{ */
		...
		$accessop = $this->params['accessobject'];
		?>
		<form class="form-horizontal" action="../op/op.RoleMgr.php" method="post" enctype="multipart/form-data" name="form"
			id="form">
			<?php
			if ($currRole) {
