...
class SeedDMS_View_UsrMgr extends SeedDMS_Theme_Style
{
	...
	{ /* {{{ */
		...
		$quota = $this->params['quota'];
		?>
		<form class="form-horizontal" action="../op/op.UsrMgr.php" method="post" enctype="multipart/form-data" name="form1"
			id="form1">
			<?php
			if ($currUser) {
			...
			foreach ($users as $usr) {
				...
				foreach ($substitutes as $r)
					if ($r->getID() == $usr->getID())
						$checked = true;
				$getName = $this->maybeDecrypt($usr->getFullName(), $encryption_key);
				$getlogin = $this->maybeDecrypt($usr->getLogin(), $encryption_key);
				$options[] = array($usr->getID(), htmlspecialchars($getlogin . " - " . $getName), $checked);
			}
			$this->formField(
					...
					foreach ($groups as $grp) {
						...
						foreach ($res as $r)
							if ($r['reviewerGroupID'] == $grp->getID())
								$checked = true;
						$getName = $this->maybeDecrypt($grp->getName(), $encryption_key);
						$options[] = array($grp->getID(), htmlspecialchars($getName), $checked);
					}
					$this->formField(
						getMLText("groups"),
						array(
				...
				if ($workflows) {
					...
					foreach ($workflows as $workflow) {
						...
						if ($mandatoryworkflows)
							foreach ($mandatoryworkflows as $mw)
								if ($mw->getID() == $workflow->getID())
									$checked = true;
	...
	{ /* {{{ */
		...
		<form class="form-horizontal">
			...
			foreach ($users as $currUser) {
				$fullNameDecrypted = $this->maybeDecrypt($currUser->getFullName(), $encryption_key);
				$getLoginDecrypted = $this->maybeDecrypt($currUser->getLogin(), $encryption_key);
				$getEmail = $this->maybeDecrypt($currUser->getEmail(), $encryption_key);

				$options[] = array(
					$currUser->getID(),
					htmlspecialchars(($getLoginDecrypted ?? '') . ' - ' . ($fullNameDecrypted ?? '')),
					$seluser && $currUser->getID() == $seluser->getID(),
					array(array('data-subtitle', htmlspecialchars($getEmail ?? '')))
				);
			}
