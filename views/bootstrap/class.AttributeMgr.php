			...
			<select class="form-control chzn-select" id="selector" class="input-xlarge">
				...
				if ($attrdefs) {
					foreach ($attrdefs as $attrdef) {
						...
						$decrypted = $this->decryptName($attrdef->getName(), $encryption_key);
						$attrdef_name = ($decrypted === '[DECRYPTION FAILED]' || $decrypted === '[INVALID NAME]') ? $attrdef->getName() : $decrypted;
						print "<option value=\"" . $attrdef->getID() . "\" " . ($selattrdef && $attrdef->getID() == $selattrdef->getID() ? 'selected' : '') . " data-subtitle=\"" . htmlspecialchars($ot . ", " . $t) . "\">" . htmlspecialchars($attrdef_name/* ." (".$ot.", ".$t.")"*/);
					}
				}
				?>
			</select>
