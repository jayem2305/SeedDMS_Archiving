			...
			if($items) {
				foreach($items as $item) {
					if($content = $item->getContent()) {
						...
						$subitems[] = array('label'=>$document->getName(), 'link'=>$this->params['settings']->_httpRoot."out/out.ViewDocument.php?documentid=".$document->getID()."&currenttab=docinfo", 'class'=>"table-row-document", 'rel'=>"document_".$document->getID());
					}
				}
			}
