...
class SeedDMS_View_Acl extends SeedDMS_Theme_Style
{
	...
	{ /* {{{ */
		...
		case -1:
		$li.find('.jqtree-element span:last-child').after('<span style="position: absolute; right:10px;"
			class="jqtree-remove-permission" data-acoid="'+node.acoid+'" data-aroid="'+node.aroid+'"><i
				class="fa fa-minus-circle"></i></span> <span style="position: absolute; right:50px;"
			class="jqtree-toggle-permission" data-acoid="'+node.acoid+'" data-aroid="'+node.aroid+'"><i
		...
		default:
		$li.find('.jqtree-element span:last-child').after('<span style="position: absolute; right:10px;"
			class="jqtree-add-permission" data-acoid="'+node.acoid+'" data-aroid="'+node.aroid+'"><i
		...
		noty({text: data.msg, type: data.type, dismissQueue: true, layout: 'topRight', theme: 'defaultTheme', timeout:
		timeout});
		},
		});
		});
	...
	{ /* {{{ */
		...
		if ($selrole) {
			...
			if (!$aro) {
				...
				?>
				<div id="acostree" data-url="out.Acl.php?action=tree&roleid=<?= ($selrole ? $selrole->getID() : 0) ?>">
					<?= getMLText('data_loading') ?></div>
				<?php
			}
		}
		$this->columnEnd();
