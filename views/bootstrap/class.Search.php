...
class SeedDMS_View_Search extends SeedDMS_Theme_Style
{
	...
	{ /* {{{ */
		...
		var url =
		$(this).attr('href')+'&changecategory='+$('#batchcategory').val()+'&removecategory='+($('#removecategory').prop('checked')
		? '1' : '0');
		var values = {};
		$('input[name^=\"marks\"]').each(function() {
		if(this.checked)
	...
	{ /* {{{ */
		...
		<ul class="nav nav-pills" id="searchtab">
			<li class="nav-item <?php echo ($fullsearch == false) ? 'active' : ''; ?>"><a
					class="nav-link <?php echo ($fullsearch == false) ? 'active' : ''; ?>" data-target="#database"
					data-toggle="tab" role="button"><?php printMLText('databasesearch'); ?></a></li>
			<?php
			if ($enablefullsearch) {
				?>
				<li class="nav-item <?php echo ($fullsearch == true && $facetsearch == false) ? 'active' : ''; ?>"><a
						class="nav-link <?php echo ($fullsearch == true && $facetsearch == false) ? 'active' : ''; ?>"
						data-target="#fulltext" data-toggle="tab" role="button"><?php printMLText('fullsearch'); ?></a></li>
				<?php
			}
			if ($enablefullsearch && $enablefacetsearch) {
				?>
				<li class="nav-item <?php echo ($facetsearch == true && $facetsearch == true) ? 'active' : ''; ?>"><a
						class="nav-link <?php echo ($facetsearch == true && $facetsearch == true) ? 'active' : ''; ?>"
						data-target="#facetfulltext" data-toggle="tab" role="button"><?php printMLText('facetfullsearch'); ?></a>
				</li>
				<?php
			}
			?>
		...
		<div class="tab-content">
			<div class="tab-pane <?php echo ($fullsearch == false) ? 'active' : ''; ?>" id="database">
				<form class="form-horizontal mb-4" action="<?= $this->params['settings']->_httpRoot ?>out/out.Search.php"
					name="form1">
