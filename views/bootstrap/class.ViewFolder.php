...
class SeedDMS_View_ViewFolder extends SeedDMS_Theme_Style
{
	...
	{ /* {{{ */
		...
		for ($i = 0; $i < count($accessList["groups"]); $i++) {
			...
			$decrypted_name = htmlspecialchars($this->decryptName(htmlspecialchars($group->getName()), $encryption_key));
			if ($decrypted_name === '[INVALID NAME]' || $decrypted_name === '[DECRYPTION FAILED]') {
				$decrypted_name = $group->getName();
			}
			$accesstext = $this->getAccessModeText($accessList["groups"][$i]->getMode());
			$content .= $accesstext . ": " . htmlspecialchars($decrypted_name);
			if ($i + 1 < count($accessList["groups"]) || count($accessList["users"]) > 0)
				$content .= "<br />";
		}
		for ($i = 0; $i < count($accessList["users"]); $i++) {
			$user = $accessList["users"][$i]->getUser();
			$decrypted_name = htmlspecialchars($this->decryptName(htmlspecialchars($user->getFullName()), $encryption_key));
			if ($decrypted_name === '[INVALID NAME]' || $decrypted_name === '[DECRYPTION FAILED]') {
				$decrypted_name = $user->getFullName();
			}
	...
	{ /* {{{ */
		...
		<?php if ($maxItemsPerPage) { ?>
			...
			url =
			seeddms_webroot+"out/out.ViewFolder.php?action=entries&folderid="+folder+"&offset="+offset+"&limit="+limit+"&orderby="+orderby;
			$.ajax({
			type: 'GET',
			url: url,
			dataType: 'json',
			...
			if(data.count <= 0) { element.hide(); } else { var str='<?= getMLText('x_more_objects') ?>' ;
				element.text(str.replace('[number]', data.count)); element.data('offset', offset+limit);
				element.prop("disabled",false); } } }); } $(window).scroll(function() { if($(window).scrollTop() +
				$(window).height() + 3>= $(document).height()) {
