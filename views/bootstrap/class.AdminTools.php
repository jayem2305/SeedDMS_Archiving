...
class SeedDMS_View_AdminTools extends SeedDMS_Theme_Style
{
	...
	{ /* Parent implementation or empty if not needed by other parts */
	}
	public function rowEnd()
	{ /* Parent implementation or empty */
	}
	public function columnStart($width = 0)
	{ /* Parent implementation or empty */
	}
	...
	private function _display_buttons_for_row($buttons_config_array, $row_index, $accessop, $settings)
	{
		...
		if ($this->hasHook('startOfRow'))
			echo $this->callHook('startOfRow', $row_index);

		foreach ($visible_buttons as $button) {
			echo $this->rowButton($button['link'], $button['icon'], $button['label']);
		}

		if ($this->hasHook('endOfRow'))
			echo $this->callHook('endOfRow', $row_index);
	...
	-->
		<style type="text/css">
			...
			#admin-tools i {
				...
				/* Apply to all elements within for consistency */
			}

			#admin-tools .btn {
				/* If any .btn class slips in from BS2 */
			...
			#admin-tools .adm-tool-item {
				...
				/* Vertical spacing between items/wrapped rows */
				display: flex !important;
				/* To allow the 'a' tag to take full height */
				flex-direction: column !important;
				/* Ensures 'a' tag behaves as block for height */
			...
			#admin-tools .adm-tool-button {
				...
				/* Horizontally center icon & text */
				width: 100% !important;
				height: 100% !important;
				/* Stretch to fill .adm-tool-item */
				min-height: 130px !important;
				/* CRITICAL: Adjust for tallest content */
			...
			#admin-tools .adm-tool-label {
				...
				/* Ensure it takes its own space for wrapping */
				max-width: 100%;
			}
		...
		<div id="admin-tools">
			<?php if ($this->hasHook('beforeRows'))
				echo $this->callHook('beforeRows'); ?>
			<?php
			// --- Render button rows ---
			...
			?>
			<?php if ($this->hasHook('afterRows'))
				echo $this->callHook('afterRows'); ?>
		</div>
		<?php
		$this->contentEnd();
		$this->htmlEndPage();
