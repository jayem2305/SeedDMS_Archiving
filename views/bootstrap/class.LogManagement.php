...
class SeedDMS_View_LogManagement extends SeedDMS_Theme_Style
{
	...
	{ /* {{{ */
		...
		if ($print_header)
			printMLText("empty_list");
		else
			print "<tr><td><span id=\"toggleall\"><i class=\"fa fa-exchange\"></i></span></td><td colspan=\"3\"><button type=\"submit\" class=\"btn btn-danger btn-mini btn-sm\"><i class=\"fa fa-remove\"></i> " . getMLText('remove_marked_files') . "</button></td></tr></table></form>\n";
	} /* }}} */
		...
		if (!$logname) {
			...
			if ($handle) {
				while ($e = readdir($handle)) {
					if (is_dir($this->logdir . $e))
						continue;
					if (strpos($e, ".log") == FALSE)
						continue;
					if (strcmp($e, "current.log") == 0)
						continue;
					$section = strtok($e, '-');
