...
class SeedDMS_View_ViewDocument extends SeedDMS_Theme_Style
{
	...
	{ /* {{{ */
		...
		if (count($files) > 0) {
			...
			$this->infoMsg(getMLText("no_attached_files"));
	} /* }}} */
			...
			<table class="table table-condensed table-sm">
				...
				if ($document->getComment()) {
					...
					<tr>
						...
						<td>
							<div class="document-comment"><?php print $comment; ?></div>
						</td>
					</tr>
					<?php
				}
		...
		if ($accessobject->check_controller_access('ViewOnline', array('action' => 'version'))) {
				...
				switch ($latestContent->getMimeType()) {
					...
					case 'audio/wav':
						...
						<audio controls style="width: 100%;" preload="false">
							<source
								src="<?= $settings->_httpRoot ?>op/op.ViewOnline.php?documentid=<?php echo $latestContent->getDocument()->getID(); ?>&version=<?php echo $latestContent->getVersion(); ?>"
								type="audio/mpeg">
						</audio>
						<?php
						break;
					...
					case 'application/pdf':
						...
						<div style="width:100%; height: 0; position:relative; padding-top: 141%;">
							<iframe
								src="<?= $settings->_httpRoot ?>pdfviewer/web/viewer.html?file=<?php echo urlencode($settings->_httpRoot . 'op/op.ViewOnline.php?documentid=' . $latestContent->getDocument()->getID() . '&version=' . $latestContent->getVersion()); ?>"
					...
					case 'image/gif':
						$this->contentHeading(getMLText("preview"));
						?>
						<img src="<?= $settings->_httpRoot ?>op/op.ViewOnline.php?documentid=<?php echo $latestContent->getDocument()->getID(); ?>&version=<?php echo $latestContent->getVersion(); ?>"
							width="100%">
						<?php
						break;
					default:
			...
			if ($converttopdf) {
				...
				if ($pdfpreviewer->hasConverter($latestContent->getMimeType())) {
					...
					<div style="width:100%; height: 0; position:relative; padding-top: 141%;">
						<iframe
							src="<?= $settings->_httpRoot ?>pdfviewer/web/viewer.html?file=<?php echo urlencode($settings->_httpRoot . 'op/op.PdfPreview.php?documentid=' . $latestContent->getDocument()->getID() . '&version=' . $latestContent->getVersion()); ?>"
							_width="100%" _height="700px"
							style="position: absolute; top: 0; left: 0; bottom: 0; right: 0; width:    100%; height: 100%"></iframe>
					</div>
					<?php
				}
