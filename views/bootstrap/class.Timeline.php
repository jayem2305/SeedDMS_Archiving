...
class SeedDMS_View_Timeline extends SeedDMS_Theme_Style
{
	...
	{ /* {{{ */
		...
		if ($data = $dms->getTimeline($from, $to)) {
			foreach ($data as $i => $item) {
				$decrypted = $this->decryptName($item['document']->getName(), $encryption_key);
				$item_name = ($decrypted === '[DECRYPTION FAILED]' || $decrypted === '[INVALID NAME]') ? $item['document']->getName() : $decrypted;
				switch ($item['type']) {
					case 'add_version':
						$msg = getMLText('timeline_full_' . $item['type'], array('document' => htmlspecialchars($item_name), 'version' => $item['version']));
						break;
					case 'add_file':
						$msg = getMLText('timeline_full_' . $item['type'], array('document' => htmlspecialchars($item_name)));
						break;
					case 'status_change':
						$msg = getMLText('timeline_full_' . $item['type'], array('document' => htmlspecialchars($item_name), 'version' => $item['version'], 'status' => getOverallStatusText($item['status'])));
						break;
	...
	{ /* {{{ */
		...
		$('div.ajax').trigger('update', {documentid: item.docid, version: item.version, statusid: item.statusid, statuslogid:
		item.statuslogid, fileid: item.fileid});
		}
		}
		}
		<?php
